<x-app-layout>
    <x-slot name="header">
        <nav class="text-sm text-gray-500 mb-4">
            ...
            <span class="text-gray-900 font-medium">Import Conflict Resolution</span>
        </nav>
    </x-slot>
    ...
    <div x-data="{ activeConflict: 0, resolved: {}, totalConflicts: {{ count($conflicts) }}, resolve(idx, value) { this.resolved[idx] = value; setTimeout(() => { if (idx < this.totalConflicts - 1) this.activeConflict = idx + 1; }, 300); } }" class="card">
        <div class="px-4 sm:px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div class="flex items-center gap-3 flex-wrap">
                <h2 class="text-lg font-bold text-gray-900">Conflict Resolution: Bulk Import</h2>
        ...
        <form method="POST" action="{{ route('suppliers.import.all.resolve') }}">
            @csrf
                ...
                <div class="w-full md:w-72 border-b md:border-b-0 md:border-r border-gray-200 bg-gray-50/50 shrink-0">
                    <div class="px-4 py-3 border-b border-gray-200">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-gray-700">Conflicts ({{ count($conflicts) }})</span>
                        </div>
                    </div>
                    <div class="p-2 space-y-1">
                        @foreach ($conflicts as $idx => $conflict)
                            ...
                            >
                                <div>
                                    <p class="text-xs text-gray-400">{{ $conflict['supplier_name'] }}</p>
                        ...
                        <div x-show="activeConflict === {{ $index }}" x-cloak class="h-full flex flex-col">
                            <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <h3 class="text-base font-bold text-gray-900">{{ $conflict['supplier_name'] }} â†’ {{ $conflict['layup_name'] }}</h3>
                            ...
                            <div class="flex-1 p-4 sm:p-6">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-4">
                                    <label class="cursor-pointer" @click="resolve({{ $index }}, 'existing')">
                                        <input type="radio" name="resolutions[{{ $conflict['supplier_index'] }}_{{ $conflict['layup_index'] }}_{{ $conflict['layer_index'] }}]" value="existing" checked class="hidden peer" />
                                    ...
                                    <label class="cursor-pointer" @click="resolve({{ $index }}, 'incoming')">
                                        <input type="radio" name="resolutions[{{ $conflict['supplier_index'] }}_{{ $conflict['layup_index'] }}_{{ $conflict['layer_index'] }}]" value="incoming" class="hidden peer" />
            ...
            <div class="px-4 sm:px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50 rounded-b-xl">
                <a href="{{ route('suppliers.index') }}" class="btn-secondary w-full sm:w-auto justify-center">
                    Cancel Import
                </a>
